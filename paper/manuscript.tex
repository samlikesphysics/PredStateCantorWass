...
\keywords{time series, predictive states, Cantor set, Wasserstein distance, hierarchical clustering}
...
\label{fig:hclust}
\Description{Four graphs are displayed. Each vertical axis has a list of
	symbolic sequences organized by a hierarchical clustering tree. The
	horizontal axis is a list of individual symbols. The content of each graph
	is rows of fractal-distributed lines.
	}
\end{figure*}
...
\Description{A rectangular scatter plot of color-coded points is displayed on
	...
	points with a diagonal line cutting through them. In the right plot the
	points are colored with the same scheme as the left, and similarly colored
	arrows point from clusters to the diagonal line or from the diagonal line
	to the next cluster.
	}
\end{figure*}
